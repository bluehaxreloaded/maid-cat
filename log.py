...
async def log_to(ctx: commands.Context, channel_id: int, title: str | None = None):
    ...
    if log_channel:
        ...
        log_embed.add_field(name="Sent by:", value=f"{ctx.message.author.name} - {ctx.message.author.id}", inline=False)
        log_embed.add_field(name="Action: ", value=ctx.message.content, inline=False) # on_message() automatically strips all content not command related
...
async def error_log(ctx: commands.Context, error: Exception):
    ...
    if error_log_channel:
        error_log_embed = discord.Embed(title="ERROR", description=str(error), color=discord.Color.red())
        
        error_log_embed.add_field(name="Sent by:", value=f"{ctx.message.author.name} - {ctx.message.author.id}", inline=False)
        error_log_embed.add_field(name="Action:", value=ctx.message.content, inline=False)
