...
class CompletionFollowUpView(discord.ui.View):
    ...
    @discord.ui.button(label="No, thank you!", style=discord.ButtonStyle.secondary, emoji="👋", custom_id="completion_no_thanks")
    async def no_thanks_button(self, button: discord.ui.Button, interaction: discord.Interaction):
    ...
    @discord.ui.button(label="Yes, I have more questions", style=discord.ButtonStyle.primary, emoji="❓", custom_id="completion_more_questions")
    async def more_questions_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if not channel:
            ...
            for ch in interaction.guild.text_channels:
                if ch.category and ch.category.id in [SOAP_CHANNEL_CATEGORY_ID, MANUAL_SOAP_CATEGORY_ID]:
        ...
        if channel:
            ...
            embed.set_footer(text="Describe in detail what's happening and please include error codes if possible.")
...
class EshopVerificationView(discord.ui.View):
    ...
    @discord.ui.button(label="Yes, the eShop works", style=discord.ButtonStyle.success, emoji="✅", custom_id="eshop_success")
    async def eshop_success_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if interaction.response.is_done():
            await interaction.followup.send(embed=completion_embed, view=view, ephemeral=True)
        else:
            await interaction.response.send_message(embed=completion_embed, view=view, ephemeral=True)

    @discord.ui.button(label="No, I need help", style=discord.ButtonStyle.danger, emoji="❕", custom_id="eshop_error")
    async def eshop_error_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        """Send assistance requested embed"""
        # Disable buttons
        for item in self.children:
            item.disabled = True
...
class SOAPAutomationCog(commands.Cog):
    ...
    async def _update_progress_message(self, target_channel: discord.TextChannel, percentage: int, footer: str = None) -> bool:
    ...
    async def on_message(self, message: discord.Message):
        ...
        progress_footers = {
            ...
            "CLEANINTY_SERIAL_CHECK": "JSON serialized...",
