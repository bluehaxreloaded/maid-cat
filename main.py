...
async def on_message(message: discord.Message):
    ...
    message.content = message.content.rsplit("|", 1)[0].strip() # trash everything not command related

    if message.content.startswith('.') and not message.content[1] == ".": # literally just so "..." doesn't flair up the bot lol
        if message.content[1].isdigit(): # variadic commands (.#)
            count = int(message.content[1])
            if count > 5:
                return await message.channel.send("A maximum of 5 commands are allowed at a time")     
            ...
            while cmds:
                ...
                if cmd == None:
                    await on_command_error(await bot.get_context(message), commands.CommandNotFound())
                else:
...
async def on_command_error(ctx: commands.Context, error: commands.CommandError):
    try:
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send(f"Error! Missing argument `{error.param.name}` in command `{ctx.command.name}`")
            ...
            await ctx.send("Error! Please don't do whatever you did again and notify someone who knows what they're doing.")
            await error_log(ctx, error)
    except ErrorLogChannelNotFound as log_error: # specific case where error_log() lacks a log to error to
