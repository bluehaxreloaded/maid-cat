    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(
                label="Yes, my 3DS is on custom firmware", value="yes", emoji="✅"
            ),
            discord.SelectOption(
                label="No, my 3DS is not on custom firmware", value="no", emoji="❌"
            ),
...
class RegionChangeView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(
                label="Yes, it's region-changed", value="yes", emoji="✅"
            ),
            discord.SelectOption(
                label="No, it's not region-changed", value="no", emoji="❌"
            ),
            discord.SelectOption(label="I'm not sure", value="unsure", emoji="❓"),
        ],
    )
    async def region_select(
        ...
        if region_answer == "no":
            ...
            embed.add_field(
                name="Region Change Guide",
                value="https://3ds.hacks.guide/region-changing.html",
                inline=False,
            )
            await interaction.response.edit_message(embed=embed, view=None)
