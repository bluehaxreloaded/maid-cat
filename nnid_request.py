...
class NNIDRequestView(discord.ui.View):
    ...
    async def request_nnid_button(
        ...
        archived_channel = (
            soap_cog.find_reopenable_channel(interaction.guild, interaction.user, is_soap=False)
            if soap_cog
            else None
        )
        if archived_channel:
            await interaction.response.defer(ephemeral=True)
            await soap_cog.reopen_channel(archived_channel, interaction.user, is_soap=False)
            embed = discord.Embed(
                title=REOPENED_TITLE,
                description=f"Welcome back! We've reopened your NNID channel.\n\n"
